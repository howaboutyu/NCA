...
learning_rate_schedule = optax.cosine_decay_schedule(
    init_value=config.learning_rate, decay_steps=config.num_epochs * config.steps_per_epoch
)
...
params = model.init(key, jax.random.normal(key, (1, 64, 64, config.model_output_len * 3)))
...
def train_step(state, key, state_grid, target, num_steps=64):
    ...
    kernel_x, kernel_y = create_perception_kernel(
        input_size=config.model_output_len, output_size=config.model_output_len, use_oihw_layout=True)
    ...
    def loss_fn(params, state_grid):
        ...
        for _ in range(num_steps):
            state_grid = cell_update(key, state_grid, model.apply, params, kernel_x, kernel_y, update_prob=0.5)

        pred_grid = state_grid[:, :3]
